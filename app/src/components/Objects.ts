...
export const setWallDataPrices = (dataPrice: any[], handlerFetch: any) => {
    dataPrice[0]?.map((item: any, index: number) => {
            if (index < 40)
                handlerFetch({
...
export const showDataDescriptionOrLoading = (scene: Scene, txt: string, position: any, rotation: any, loading?: any) => {
    if (txt) {
        let font_type = "Arial";
...
export const showDataPrices = (scene: Scene, dataPrice: any) => {
    setWallDataPrices(dataPrice, createImagePlane)
    if (dataPrice[0]?.length != 0) {
        dataPrice[0]?.map((item: any, index: any) => {
                if (index < 36) {
                    showDataDescriptionOrLoading(scene, dataPrice[0][index]?.name,
                        new Vector3(
